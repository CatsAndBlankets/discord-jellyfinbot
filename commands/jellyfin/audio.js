...
module.exports = {
    data: {
        name: 'audio',
        description: 'displays options to change language for the media currently playing (if any)',
        folder: 'jellyfin'
    },
    async execute (interaction) {
            ...
            if (i.Type === 'Audio') {
                if (subtitleTracks.length <= 10) {
                    subtitleTracks.push({
                        ...
                        Index: i.Index,
                        Set: i.Index === playState.SubtitleStreamIndex ? true : false
                    })
        ...
        if (subtitleTracks.length === 1) {
            return await interaction.channel.send(`There are no options to change audio for this ${nowPlaying.Type.toLowerCase()}`)
        }
